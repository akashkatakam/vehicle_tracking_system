...
def render():
    ...
    if 'scanned_chassis' not in st.session_state:
    ...
    try:
        ...
        if task_display_str:
            ...
            # --- NATIVE CAMERA INPUT ---
            img_file_buffer = st.camera_input("Take a photo of the QR Code", key="mechanic_cam")
            ...
            if img_file_buffer is not None:
                ...
                if scanned_code:
                    ...
                    st.warning("No QR code detected. Try getting closer or tapping to focus.")
                ...
                if not chassis_val:
                    ...
                    try:
                        ...
                        if success:
                            st.success(message)
                            st.balloons()
                            # Clear the scan for the next task
                            st.session_state.scanned_chassis = ""
                            st.cache_data.clear()
                            st.rerun()
                        else:
